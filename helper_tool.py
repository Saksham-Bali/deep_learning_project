...
class ConfigToronto3D:
    ...
    num_points = 32768  # Number of input points
...
class DataProcessing:
    ...
    def knn_search(support_pts, query_pts, k):
        ...
        # Note: This is slower than C++ implementation but works without compilation
        neighbor_idx = []
        for i in range(support_pts.shape[0]):
            nbrs = NearestNeighbors(n_neighbors=k, algorithm='kd_tree').fit(support_pts[i])
            _, indices = nbrs.kneighbors(query_pts[i])
            neighbor_idx.append(indices)
        return np.array(neighbor_idx, dtype=np.int32)
    ...
    def grid_sub_sampling(points, features=None, labels=None, grid_size=0.1, verbose=0):
        ...
        """
        raise NotImplementedError("C++ grid_sub_sampling is not available. Please use pre-processed data.")
