...
class Network:
    ...
    def inference(self, inputs, is_training):
        ...
        feature = tf.nn.leaky_relu(tf.keras.layers.BatchNormalization(axis=-1, momentum=0.99, epsilon=1e-6)(feature, training=is_training))
        ...
        for i in range(self.config.num_layers):
            # Dynamic KNN for last 3 layers (2, 3, 4) - feature-space neighbors
            # For first 2 layers (0, 1), use pre-computed XYZ-based neighbors
            if i in [2, 3, 4]:
                # Squeeze feature to [B, N, C]
                curr_feature = tf.squeeze(feature, axis=2)
                
                # Compute dynamic KNN in feature space
                k = self.config.k_n
                neigh_idx = self.knn(curr_feature, k)
            else:
                # Use pre-computed XYZ-based neighbors
                neigh_idx = inputs['neigh_idx'][i]
                
            # Apply dilated residual block with the computed neighbors
            # (either dynamic from features or static from XYZ)
                
            f_encoder_i = self.dilated_res_block(feature, inputs['xyz'][i], neigh_idx, d_out[i],
    ...
    def knn(self, feature, k):
        ...
        # dist(i, j) = ||x_i - x_j||^2 = ||x_i||^2 + ||x_j||^2 - 2*x_i*x_j
        
        r = tf.reduce_sum(feature * feature, axis=2, keepdims=True) # [B, N, 1]
        # Transpose for broadcasting
        r_t = tf.transpose(r, perm=[0, 2, 1]) # [B, 1, N]
        
        inner = tf.matmul(feature, feature, transpose_b=True) # [B, N, N]
        
        # Distance matrix
        dist = r - 2 * inner + r_t # [B, N, N]
        
        # Find k nearest neighbors (smallest distance)
        # top_k finds largest, so we use negative distance
