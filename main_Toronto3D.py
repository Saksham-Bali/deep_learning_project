...
# Patch for sklearn < 1.0 compatibility with pickles
try:
    from sklearn.neighbors import _kd_tree
    sys.modules['sklearn.neighbors.kd_tree'] = _kd_tree
except ImportError:
    pass

try:
    from sklearn.neighbors import dist_metrics
    sys.modules['sklearn.neighbors.dist_metrics'] = dist_metrics
except ImportError:
    try:
        from sklearn.metrics import _dist_metrics
        sys.modules['sklearn.neighbors.dist_metrics'] = _dist_metrics
    except ImportError:
        pass
...
class Toronto3D:
    ...
    def load_sub_sampled_clouds(self, sub_grid_size, mode):
        ...
        for i, file_path in enumerate(files):
            ...
            # Calculate density
            # Density is inversely proportional to the mean distance to nearest neighbors
            # We use k=16 (same as cfg.k_n)
            # Note: search_tree.data contains the points
            points = np.array(search_tree.data, copy=False)
            # Query the tree for k nearest neighbors
            # The first neighbor is the point itself, so we query k+1
            dists, _ = search_tree.query(points, k=cfg.k_n + 1)
            ...
            if not hasattr(self, 'input_density'):
                self.input_density = {'training': [], 'validation': [], 'test': []}
            self.input_density[cloud_split] += [density]
    ...
    def get_batch_gen(self, split):
        ...
        # Random initialize or Class-balanced initialize
        for i, tree in enumerate(self.input_trees[split]):
            if split == 'test':
                self.possibility[split] += [np.random.rand(tree.data.shape[0]) * 1e-3]
            else:
                # Initialize possibility based on class frequency (rare classes -> low possibility -> picked first)
                labels = self.input_labels[split][i]
                # Map labels to weights
                # Note: labels are original labels, need to map to 0..N-1 if they are not
                # But self.label_to_idx is used later. Let's check input_labels.
                # input_labels are loaded from ply 'class' field.
                # In load_sub_sampled_clouds: sub_labels = data['class']
                # In get_batch_gen loop: queried_pc_labels = [self.label_to_idx[l] for l in queried_pc_labels]
                # So input_labels are raw labels.
                # We need to map them to indices to use class_weight.
                mapped_labels = np.array([self.label_to_idx[l] for l in labels])
                
                # Initialize possibility based on density: High density -> High possibility -> Picked less
                # This ensures better coverage of sparse regions
                density = self.input_density[split][i]
                # Invert density so sparse regions (low density) get low possibility (picked first)
                point_weights = 1.0 / (density + 1e-6)  # Add epsilon to avoid division by zero
                # Normalize to reasonable range
                point_weights = point_weights / np.max(point_weights)
                
                # Add small random noise
                self.possibility[split] += [point_weights + np.random.rand(tree.data.shape[0]) * 1e-3]
        ...
        def spatially_regular_gen():
            ...
            for i in range(num_per_epoch):  # num_per_epoch
                ...
                # Multiply by density weight (high density -> larger delta -> picked less often next time)
                density_weight = self.input_density[split][cloud_idx][query_idx]
                delta = delta * density_weight
...
if __name__ == '__main__':
    ...
    if Mode == 'train':
        ...
        if FLAGS.model_path is not 'None':
            print(f'Resuming training from checkpoint: {FLAGS.model_path}')
